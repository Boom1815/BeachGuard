-- ═══════════════════════════════════════════════════════════════════
-- TEST DU SCHÉMA — à exécuter APRÈS schema_beachguard.sql
...
begin
  ...
  values (payer_id, '+32470000001', 'authenticated', 'authenticated');

  -- 2. Vérifie que le profil a bien été créé automatiquement
  if not exists (select 1 from public.profiles where id = payer_id) then
  ...
  values (payer_id, '+32470000002');

  select contact_user_id is null into strict linked_count
  from public.relations where payer_id = payer_id;
  ...
  insert into auth.users (id, phone, aud, role)
  values (contact_id, '+32470000002', 'authenticated', 'authenticated');

  -- 5. Vérifie que la relation a été liée automatiquement
  select count(*) into relation_count
  from public.relations
  where payer_id = payer_id and contact_user_id = contact_id;
  ...
  delete from auth.users where id in (payer_id, contact_id);
